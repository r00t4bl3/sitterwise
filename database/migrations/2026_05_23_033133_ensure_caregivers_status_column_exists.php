...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (Schema::hasTable('caregiver_statuses') && Schema::hasColumn('caregivers', 'status_id')) {
            DB::statement("UPDATE caregivers SET status = (SELECT LOWER(REPLACE(name, ' ', '_')) FROM caregiver_statuses WHERE caregiver_statuses.id = caregivers.status_id) WHERE EXISTS (SELECT 1 FROM caregiver_statuses WHERE caregiver_statuses.id = caregivers.status_id)");
    ...
    {
        if (Schema::hasTable('caregivers') && ! Schema::hasColumn('caregivers', 'status_id')) {
