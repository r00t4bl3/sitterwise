...
- **Cleanup command:** `php artisan app:cleanup-junk-children [--dry-run] [--purge]`. Soft-deletes `client_children` rows with a junk name (`None`/`N/A`/`no kids`/… blocklist) and no birth data; strips junk + null entries from `booking_groups.children` JSON (and empties a group whose `children` is a junk string) via **direct `DB::table` updates to bypass the auto-repricing observer**. `--dry-run` reports counts without writing. `--purge` permanently force-deletes matching rows **including ones already soft-deleted by a prior run** (recycle-bin sweep) — targeted to the same junk criteria, so unrelated soft-deleted children are left alone. **→ run on prod: `--dry-run`, then default (soft-delete), verify, then `--purge`.**
  - **Service-type rule (prod-driven):** for `petsitter`/`companion_care` groups (child-free services), **any birthless child entry is junk** — including count-only `Child 2`/`Child 3` placeholders — because those services shouldn't have children at all. Every other service keeps its placeholders (a babysitter "Child 2" is a real un-named kid). Entries with real birth data are always kept, even on a pet/companion booking (likely a mis-categorised booking, not junk). Prod breakdown of the 889 `%Child %` groups: babysitter 514 / corporate_invoiced 184 / **petsitter 126** / **companion_care 21** / group_childcare_invoiced 10 → ~147 groups cleaned by this rule.
  - **✅ Ran & verified on prod:** `client_children rows removed: 0` (junk row cleared on an earlier run), `booking_groups updated: 154 (215 entries)`. Post-run checks: petsit/companion groups with placeholder children → **0**; babysitter/corporate/group-childcare placeholders → **544 / 184 / 14, unchanged**. Real children untouched; petsit/companion junk gone.
